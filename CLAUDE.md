...
**Handler method names must be globally unique across every `IEndpointGroup`, not just within one class.** `MapGet`/`MapPost`/etc. use the delegate's method name as both the OpenAPI `operationId` and the ASP.NET Core endpoint `Name`, and endpoint names must be unique app-wide. Two endpoint groups with a same-named handler (e.g. two `GetUsers` methods, one in `Users.cs` and one in `Reports.cs`) compiles fine but throws `System.InvalidOperationException: Duplicate endpoint name` at the *first incoming request* (lazy router-matcher init) — this takes down routing for **every** endpoint in the app, not just the colliding ones. Because the crash happens before CORS middleware can attach headers, the browser reports it as a CORS error, not a 500 — if the frontend suddenly gets CORS-blocked on everything after adding an endpoint, check for a duplicate handler method name first, via `grep -rn "public static async Task<" src/Web/Endpoints/*.cs` and diffing method names. When a new group's endpoint is conceptually a "GetX" that already exists elsewhere (e.g. a report that reuses an existing query), name the handler method distinctly (e.g. `GetUsersReport` instead of `GetUsers`).
